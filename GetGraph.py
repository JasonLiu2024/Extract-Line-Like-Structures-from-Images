...
def find_color(im: Image, rgb: Tuple[int]) -> np.ndarray:
    ...
    px = np.asarray(im)
    # print(f"find_color::px  {px.shape}")
    x, y, channels = px.shape
    out = np.zeros((x, y), dtype=np.uint8)
    # print(f"find_color::out {out.shape}")
    r, g, b = rgb
